...
def f_limpiar(ventana):
    ...
    ventana.carrera_texto.delete(0,END)
...
def f_nuevo(ventana):
    ...
    carrera=ventana.carrera_texto.get()
    ventana.tabla_datos.insert("",END,text=nombre,values=(apellido,carrera))
...
def f_actualizar(ventana):
    if ventana.nombre_texto.get()=="":
        ...
        if mensaje == True:
            ...
            return ventana.tabla_datos.item(elem_actualizar,text=nombre,values=(apellidos,carrera))
        else:
